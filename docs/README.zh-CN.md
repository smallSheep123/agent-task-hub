...
ZCode 使用已安装桌面端自带的 App Server。适配器在运行时读取现有 ZCode 账户配置，只在内存中按 ZCode 本机格式解密；不会把供应商凭证复制到 Hub 配置、状态、日志或仓库。Telegram 新建或继续的任务会同步到 ZCode Desktop 的本地任务索引，Hub 也会同时显示普通会话与 fork 会话。Telegram 发起的会话会保持实时订阅，用于完成通知、审批和问题处理；服务重启后由轮询作为恢复通道。桌面端手动任务由本地索引检测完成；若能按会话和完成时间匹配到本机数据库中的最终助手消息，Telegram 完成通知也会显示最近回复。
